...
data.frame(`length` = dat) %>% 
  ggplot(aes(x = length)) +
  geom_histogram(bins = 9, colour = "white") +
...
Данные `r sort(dat)`, медиана 6

![:scale 45%](img/lizards-odd.png)

Если четное число наблюдений, то медиана находится между двумя центральными (их среднее арифметическое).

Данные `r c(sort(dat), 14)`, медиана 7.5

![:scale 45%](img/lizards-even.png)
...
__Дисперсия__ (средний квадрат отклонений) — характеристика разброса. Дисперсия в выборке обозначается $s^2$, в генеральной совокупности — $\sigma^2$.


<br/>

Для данных из примера дисперсия $s^2 = \frac{`r sum((dat - mean(dat))^2)`}{11 - 1} = `r round(var(dat), 2)`$
...
__Стандартное (среднеквадратичное) отклонение__ — это характеристика разброса, средняя величина отклонения от среднего значения. Стандартное отклонение в выборке обозначается $s$, в генеральной совокупности — $\sigma$.
...
gg_perc <- data.frame(`length` = dat) %>% 
  ...
  geom_rug(aes(x = length), size = 2, colour = "black", alpha = 0.5, length = unit(0.02, "npc")) +
  labs(colour = "", x = "Длина ящериц, см", y = "Частота", title = "Персентили") + 
  scale_x_continuous(breaks = 3:13)
...
__Персентиль__ — частный случай квантиля. Всего 99 персентилей, они делят ряд наблюдений на 100 частей.
...
Персентиль показывает __какой процент__ данных меньше или равен ему:
...
А что если нарисовать на графике эти персентили последовательно?
