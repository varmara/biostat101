...
- Теорема Байеса
- Колоколообразные кривые. Нормальное распределение
...
```{r g-norm, opts.label='medium.taller'}
...
```{r gg-norm-hist-curve, opts.label='medium.taller'}
set.seed(39025)
N <- 100000
binwidth <- 0.5
mu <- 10
...
ND_curve + 
  ...
  annotate(geom = 'text', parse = TRUE, size = 9,
           x = mu, y = 0.05,
...
```{r g-norm-interval, opts.label='medium.taller'}
x1 <- 11
x2 <- 14
ND_curve + 
    stat_function(geom = 'area', fun = dnorm, args = list(mean = mu, sd = sig), 
...
```{r gg-x3, opts.label='medium.taller'}
x3 = 7
gg_x3 <- ND_curve + 
...
Cобытия "курение" и "рак легких" — зависимые, и правило умножения не выполняется:
...
А если ошибочно считать по формуле для независимых событий

$P(курение) \times P(рак) = 0.685 \times (0.613 + 0.015) = 0.43018$.
...
# Колоколообразные кривые<br/>Нормальное распределение
