...
- `r n` человек случайным образом распределили по двум вариантам эксперимента (снотворное 1 или 2).
...
![](img/paired-samples.png)

- Наблюдения взаимозависимые (парные). 
...
**Осторожно!** Если не учитывать зависимость между группами, можно прийти к неверному выводу.
...
```{r echo=FALSE, opts.label='fig.wider.taller'}
library(car)
par(cex = 1.4)
...
Для этих данных хорошо бы использовать другой, непараметрический тест, у которого нет таких жестких требований к данным.
...
Тогда разница выборочных средних нормально распределена
...
__Если предположить, что дисперсии в группах равны__, то  
$$SE_{\bar{x}_1 - \bar{x}_2} = \sqrt{\sigma^2 \big( \frac{1}{n_{1}} + \frac{1}{n_{2}}\big)}$$

--

Тогда $\sigma^2$ можно оценить так:

$$s_p^2 = \cfrac{(n_1 - 1)s^2_1 + (n_2 - 1)s^2_2}{(n_1 - 1) + (n_2 - 1)}$$

$s_p^2$ — это __обобщенная дисперсия__, т.е. усредненная дисперсия в двух выборках, взвешенная по их числу степеней свободы.
...
- t-тест Стьюдента --- если считать, что дисперсии в группах равны (как мы это делали для расчета доверительного интервала)
...
$s_p^2 = \cfrac{(n_1 - 1)s^2_1 + (n_2 - 1)s^2_2}{(n_1 - 1) + (n_2 - 1)}$ --- это __обобщенная дисперсия__ по двум выборкам.

Результирующая $t$-статистика подчиняется $t$-распределению с  $df = n_1 + n_2 - 2$.

--

Равенство дисперсий в группах --- это часто нереалистичное предположение!
...
$$SE_{\bar{x}_1 - \bar{x}_2} = \sqrt{ {s^2_{1}}/{n_{1}} + {s^2_{2}}/{n_{2}}}$$

$$df_{ WS} \approx \cfrac {\big({s^2_{1}}/{n_{1}} + {s^2_{2}}/{n_{2}}\big)^2}
{\frac{1}{n_{1} - 1}\big(\frac {s_{1}^2} {n_{1}}\big)^2 + \frac{1}{n_{2} - 1}\big(\frac {s_{2}^2} {n_{2}}\big)^2}$$
...
$$df_{ WS} \approx \frac {\big({\frac{`r sd1`^2}{`r n1`} + \frac{`r sd2`^2}{`r n2`}}\big)^2}
{\frac{1}{`r n1` - 1}\big(\frac {`r sd1`^2} {`r n1`}\big)^2 + \frac{1}{`r n2` - 1}\big(\frac {`r sd2`^2} {`r n2`}\big)^2} = \\ = `r dfws`$$
...
(назван в честь Фишера Snedecor, Cochran, 1989)
...
Условие применимости: нормальное распределение в обеих группах.
...
Условие применимости: распределение значений во всех группах приблизительно симметрично. 
