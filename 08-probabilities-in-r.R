...
# В лесах Миссури по результатам исследований 45 из 273 деревьев 
# были с дуплами (Fan et al. 2005).
# 
# - Какова вероятность того, что случайно выбранное дерево на этой территории 
# будет с дуплом?
...
# Визуализируем это распределение (просто для себя)
library(ggplot2)
# Данные для графика
dfr <- data.frame(x = seq(from = 150, to = 200, length.out = 1000))
# График распределения роста пилотов
...
# 1. Какова вероятность того, что случайно выбранный мужчина 
...
# 2. Какова вероятность того, что случайно выбранный мужчина 
...
# 3. Какова в популяции доля мужчин, подходящих по росту в пилоты, 
# т.е. __больше 160 и меньше 190 см__?
